...
export function imgToPicture(userOptions = {}) {
  ...
  const generateImageVariants = async (imagePath, baseName, ext, metadata) => {
    const variants = []
    const formatTasks = []
    const baseWidth = options.isRetinaSupport ? Math.round(metadata.width / 2) : metadata.width
      ...
      formatTasks.push(
        (async () => {
          ...
          if (options.isDebugMode) {
            console.log(`Generated ${outputFilename} (${variant.width}x${variant.height})`)
          }
  ...
  const processImagesInContent = async (content, baseDir, fileId) => {
    ...
    for (const match of matches) {
      ...
      try {
        ...
        const imgSrcVariant =
          originalFormatVariants.find(v => v.density === '1x') || originalFormatVariants[0]
  ...
  return {
    ...
    async transform(code, id) {
      ...
      try {
        ...
        processedContent = await processImagesInContent(processedContent, baseDir, id)
    ...
    async closeBundle() {
      if (imageStats.length > 0) {
        ...
        const maxSizeLength = Math.max(...imageStats.map(stat => formatFileSize(stat.size).length))
